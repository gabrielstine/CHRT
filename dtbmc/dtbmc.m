function fitResut = dtbmc(data,fitOptions,varargin)
%DTBMC apply DTB or Monte Carlo method to fit 1D signed coherence data
%   fitResut = dtbmc(data,fitOptions,varargin)
...
newFitOpt = struct('upBoundaryProfile',ubp,...    
    ...
    'isMonteCarlo',strcmp(fitOptions.fitType,'Monte Carlo'),... % Use Monte Carlo method.
    'rngSeed',rngSeed,...    
...
switch optMethod
    case 'fminsearchbnd'
        [theta,fval,exitflag,output] = fminsearchbnd(@(theta) newFit801_bnd(theta,data,newFitOpt),...
            tg,tl,tu);
        fitResut = struct('theta',theta(1:10),...
            'upperBoundaryParameter',theta(11:15),...
            'lowerBoundaryParameter',theta(16:20),...
            'fval',fval,...
            'exitflag',exitflag,...
            'output',output);                                            
    ...
    case 'fmincon'
        options = optimoptions(@fmincon,...
            ...
            'FinDiffType','central',...
            'UseParallel','always');
        [theta,fval,exitflag,output] = fmincon(@(theta) newFit801_bnd(theta,data,newFitOpt),...
            tg,[],[],[],[],tl,tu,[],options);
        fitResut = struct('theta',theta(1:10),...
            'upperBoundaryParameter',theta(11:15),...
            'lowerBoundaryParameter',theta(16:20),...
    ...
    case 'globalsearch'
        ...
        problem = createOptimProblem('fmincon',...
            'x0',tg,...
            'objective',@(theta) newFit801_bnd(theta,data,newFitOpt),...
            'lb',tl,...
            'ub',tu,...
            'options',options);
        gs = GlobalSearch;
    ...
    case 'patternsearch'
        ...
        options = psoptimset(options,...
            ...
            'Vectorized','off',...
            'UseParallel','always');
        [theta,fval,exitflag,output] = patternsearch(@(theta) newFit801_bnd(theta,data,newFitOpt),...
            tg,[],[],[],[],tl,tu,[],options);
        fitResut = struct('theta',theta(1:10),...
            'upperBoundaryParameter',theta(11:15),...
            'lowerBoundaryParameter',theta(16:20),...                        
